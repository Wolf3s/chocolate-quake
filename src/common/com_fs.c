...
static void COM_CopyFile(char* netpath, char* cachepath) {
    ...
    while (remaining) {
        int count = SDL_min(remaining, sizeof(buf));
...
static void COM_UseCustomPaths(int arg_num) {
    com_modified = true;
    com_searchpaths = NULL;
    for (int i = arg_num; i < com_argc; i++) {
        char* arg = com_argv[i];
        if (!arg || *arg == '+' || *arg == '-') {
            break;
        }
